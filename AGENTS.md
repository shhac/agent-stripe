...
- Use `make apicheck` to validate the requests the CLI emits against Stripe's published OpenAPI spec (paths and query parameter names). A test walks the command tree and fails if a command reaching Stripe has no entry in the table (or in its `notNetworked` list), so new commands cannot slip past unvalidated. Stripe does not publish the `/v2` namespace in that spec, so v2 requests are listed rather than validated.
