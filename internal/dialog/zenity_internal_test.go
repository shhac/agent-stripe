...
func TestStepTitle(t *testing.T) {
	cases := []struct {
		...
		{"single field — no annotation", "agent-stripe credential: foo", 0, 1, "agent-stripe credential: foo"},
...
func TestClassifyZenityErrorPreservesCancelSentinel(t *testing.T) {
	...
	wrapped := classifyZenityError(errors.New("connection failed"), Field{Label: "Stripe API key"})
	if wrapped == nil {
