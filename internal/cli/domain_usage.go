...
WHICH ACCOUNTS NEED ATTENTION?
  ...
  Neither account list endpoint returns capability or requirement detail, so the sweep retrieves
  each account and reports only the blocked ones. Keep --limit modest.

WHAT IS A REQUIREMENT ASKING FOR?
  "external_account" -> agent-stripe accounts external-accounts acct_...    (v1 endpoint; Accounts v2
                        has no external-accounts hash, so this is the one to use in either namespace)
  a person           -> agent-stripe accounts persons list acct_...          (Connect v1)
                        agent-stripe accounts-v2 persons list acct_...       (Accounts v2)
  a capability       -> agent-stripe accounts capabilities acct_...          (v1 Capability objects
                        carry their own requirements; the Account object only carries statuses)
  a payout route     -> agent-stripe accounts-v2 payout-methods acct_...     (v2 recipients; preview API,
                        so pass --v2-api-version <preview train>)
